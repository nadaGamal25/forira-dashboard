...
export default function Villages() {
  ...
  async function getData() {
    try {
      ...
      setdata(response.data.data.village)
  ...
  async function sendDataToApi() {
    try {
      const response = await axios.post(`https://delivery-app-pi-sable.vercel.app/api/village`,
       newData,
      {
        headers: {
...
<div className=" py-3">
  <div className="edit-form">
    <div className="p-saee p-3">
      <h5 className="text-center mb-3">إضافة قرية جديدة   </h5>
...
onClick={() => {
  if (window.confirm('هل انت بالتأكيد تريد الحذف ؟')) {
    axios
      .delete(`https://delivery-app-pi-sable.vercel.app/api/village/${item._id}`, 
       {
        headers: {
          token: localStorage.getItem('userToken'),
        },
